    ...
    with torch.no_grad():
            for i, (X_test, Y_test) in enumerate(tqdm(testDatasetLoader, desc="Evaluating", total=len(testDatasetLoader))):
                ...
                if generate_overlays and overlay_dir:
                    ...
                    resized_input_image = cv2.resize(input_image, (4096, 3000))  # Resize to fixed size of 3000x4096
                    
                    # Resize predicted and label masks to 3000x4096
                    predicted_mask = cv2.resize(preds[0][0] * 255, (4096, 3000), interpolation=cv2.INTER_NEAREST).astype(np.uint8)
                    label_mask = cv2.resize(labels[0][0] * 255, (4096, 3000), interpolation=cv2.INTER_NEAREST).astype(np.uint8)

                    # Find contours and overlay them
                    ...
                    overlay_pred = resized_input_image.copy()
                    cv2.drawContours(overlay_pred, contours_pred, -1, (0, 0, 255), 2)  # Red for prediction
                    overlay_label = resized_input_image.copy()
                    cv2.drawContours(overlay_label, contours_label, -1, (0, 255, 0), 2)  # Green for label

                    # Stack both overlays horizontally
                    stacked_image = cv2.hconcat([overlay_pred, overlay_label])

                    # Write accuracy and other metrics on the image
                    accuracy_text = f"Accuracy: {current_accuracy:.2f}%"
                    precision_text = f"Precision: {current_precision:.2f}%"
                    recall_text = f"Recall: {current_recall:.2f}%"
                    f1_score_text = f"F1 Score: {current_f1score:.2f}%"
                    cv2.putText(stacked_image, accuracy_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
                    cv2.putText(stacked_image, precision_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
                    cv2.putText(stacked_image, recall_text, (10, 95), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (95, 113, 70), 2)
                    cv2.putText(stacked_image, f1_score_text, (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (94, 66, 109), 2)
...
if __name__ == "__main__":
    model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/width/run_clamp_2/clamp_width_SegNextV2_2024-12-10_accuracy_train_0.96593_loss_0.01743_epoch_62_lr_0.003_time_1164.86M_alpha_0.7_beta_0.3_gamma_2_best.pth" 
    data_path = "/home/saqib/Projects/sebang_welding/Sebang_Latest_Dec24/clamp/weld"
    save_dir = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/width/run_clamp_2"  # Update with your desired folder to save outputs
    overlay_dir = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/width/run_clamp_2/overlays_test_original_image_size"  # Directory to save overlays if enabled
