...
def train(load_pre_trained_model=True):
    ...
    trainDataset = WeldingWidthDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Al/weld",
    ...
    validDataset = WeldingWidthDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Al/weld",
    ...
    pretrained_model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_2025/busbar_al/weld/run_busbar_al_1_W640_H128/busbar_al_width_SegNextV2_2025-03-26_Acc_0.986947693_loss_0.006578032_E272_time_1245.7M_W640_H128_best.pth"
    ###################################################################################################################################
    ...
    dataset_name = "busbar_al_width"
    build_date = str(date.today())
    model_name = f"{dataset_name}_{model_arch}_{build_date}"
    date_dir = os.path.join(Base_dir,'trainedModel/Phase_2025/busbar_al/weld')
    ...
    run_dir = os.path.join(date_dir, f"run_busbar_al_{run_number}_W{imageWidth}_H{imageHeight}")
    os.makedirs(run_dir)
    ...
    for epoch in range(epochs):
        ...
        if avg_acc > best_train_acc:
            ...
            best_model_path = os.path.join(run_dir,f"{model_name}_Acc_{best_train_acc:.9f}_loss_{avg_cost:.9f}_E{epoch+1}_time_{Total_training_time/60:.1f}M_W{imageWidth}_H{imageHeight}_best.pth")
    ...
    last_model_path = os.path.join(run_dir,f"{model_name}_train_{avg_acc:.9f}_loss_{avg_cost:.9f}_epoch_{epoch+1}_lr_{learningRate:.3f}_time_{Total_training_time/ 60:.1f}M_W{imageWidth}_H{imageHeight}_last.pth")
