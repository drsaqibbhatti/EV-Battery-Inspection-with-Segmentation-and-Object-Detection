...
from util.helper import FocalTverskyLoss
...
def train(load_pre_trained_model=True):
    ...
    trainDataset = PinholeDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Cu/inspection/dataset",
    ...
    validDataset = PinholeDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Cu/inspection/dataset",
    ...
    dataset_name = "cu_pinhole"
    build_date = str(date.today())
    model_name = f"{dataset_name}_{model_arch}_{build_date}"
    date_dir = os.path.join(Base_dir, 'trainedModel/Phase_2025/busbar_cu/inspection')
    ...
    run_dir = os.path.join(date_dir, f"run_busbar_cu_{run_number}_W{imageWidth}_H{imageHeight}")
    os.makedirs(run_dir)
    ...
    if load_pre_trained_model:
        ...
        pretrained_model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_2025/busbar_cu/inspection/run_busbar_cu_1_W1440_H360/cu_pinhole_SegNextV2_2025-03-24_Acc_0.817223250_loss_0.101017147_E114_time_1314.24M_best_W1440_H360.pth"
    ...
    for epoch in range(epochs):
        ...
        print('Epoch: %01d, Train cost: %.9f, Train accuracy: %.9f,Pixel accuracy: %.9f, Val cost: %.9f, Val accuracy: %.9f, learning rate: %.5f' % (epoch + 1, avg_cost, avg_acc, avg_acc_pixel, val_cost, val_acc, learningRate))
        ...
        if avg_acc > best_train_acc:
            ...
            best_model_path = os.path.join(run_dir,f"{model_name}_Acc_{best_train_acc:.9f}_loss_{avg_cost:.9f}_E{epoch+1}_time_{Total_training_time / 60:.2f}M_best_W{imageWidth}_H{imageHeight}.pth")
    ...
    last_model_path = os.path.join(run_dir,f"{model_name}_Acc_{avg_acc:.9f}_loss_{avg_cost:.9f}_E{epoch+1}_time_{Total_training_time / 60:.2f}M_W{imageWidth}_H{imageHeight}last.pth")
