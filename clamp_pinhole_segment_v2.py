...
from model.CustomSegmentationV5 import CustomSegmentationV5
from torchsummary import summary
from util.helper import IOU
from util.helper import DiceLoss
from util.helper import FocalTverskyLoss, apply_motion_blur
...
def train():
    ...
    trainDataset = PinholeDataset(path="/home/saqib/Projects/sebang_welding/dataset_phase_2/Clamp_inspection/dataset",
    ...
    validDataset = PinholeDataset(path="/home/saqib/Projects/sebang_welding/dataset_phase_2/Clamp_inspection/dataset",
    ...
    CustomSegmentation = CustomSegmentationV5().to(device)
    print('==== model info ====')
    ...
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, threshold=5e-3, threshold_mode='rel', patience=4, verbose=True, min_lr=0.001)
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0009, max_lr=0.003, step_size_up=totalTrainBatch*5, mode='triangular')

    # Initialize variables to track the best model
    best_train_acc = 0.0  # Or use np.inf if tracking the lowest loss
    best_model_path = None
    Base_dir='/home/saqib/Projects/sebang_welding/'
    model_arch = "CustomSegmentationV5"
    dataset_name = "clamp"
    build_date = str(date.today())
    model_name = f"{dataset_name}_{model_arch}_{build_date}_accuracy"
    date_dir = os.path.join(Base_dir, 'trainedModel/Phase_2/clamp/pinhole')
    ...
    run_dir = os.path.join(date_dir, f"run_clamp_{run_number}_alpha_{alphaa}_beta_{betaa}_gamma_{gammaa}")
    os.makedirs(run_dir)
    metrics=[]
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(trainDatasetLoader), desc=f"Epoch {epoch+1}/{epochs}", unit="batch") as pbar:
            for X, Y in trainDatasetLoader:
                ...
                pbar.update(1)
                pbar.set_postfix(cost=cost.item(), accuracy=accuracy, pixel_accuracy=pixel_accuracy,learningRate=current_lr)
        ...
        print('Epoch: %01d, Train cost: %.9f, Train accuracy: %.9f,Train Pixel accuracy: %.9f, Val cost: %.9f, Val accuracy: %.9f, learning rate: %.9f' % (epoch + 1, avg_cost, avg_acc,avg_acc_pixel, val_cost, val_acc, current_lr))
        ...
        if avg_acc > best_train_acc:
            ...
            best_model_path = os.path.join(run_dir,f"{model_name}_train_{best_train_acc:.5f}_loss_{avg_cost:.5f}_epoch_{epoch+1}_lr_{current_lr:.3f}_time_{Total_training_time}M_alpha_{alphaa}_beta_{betaa}_gamma_{gammaa}_best.pth")
    ...
    last_model_path = os.path.join(run_dir,f"{model_name}_train_{avg_acc:.5f}_loss_{avg_cost:.5f}_epoch_{epoch+1}_lr_{current_lr:.3f}_time_{Total_training_time}M_alpha_{alphaa}_beta_{betaa}_gamma_{gammaa}_last.pth")
