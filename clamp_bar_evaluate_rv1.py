...
from util.BarDataset import BarDataset
...
def evaluate_model(model_path, data_path, save_dir, generate_overlays=False, overlay_dir=None):
    ...
    testDataset = BarDataset(path=data_path,
    ...
    with torch.no_grad():
        for i, (X_test, Y_test) in enumerate(tqdm(testDatasetLoader, desc="Evaluating", total=len(testDatasetLoader))):
            ...
            if generate_overlays and overlay_dir:
                ...
                # Create a mask for the predicted area
                ...
                cv2.drawContours(overlay_pred, contours_pred, -1, (0, 0, 255), 2)  # Draw red contours for prediction

                overlay_label = input_image.copy()
                cv2.drawContours(overlay_label, contours_label, -1, (0, 255, 0), 2)  # Draw green contours for label
                ...
                cv2.putText(stacked_image, accuracy_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                cv2.putText(stacked_image, precision_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                cv2.putText(stacked_image, recall_text, (10, 95), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (95, 113, 70), 2)
                cv2.putText(stacked_image, f1_score_text, (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (94, 66, 109), 2)
    ...
    sns.heatmap(conf_matrix_normalized * 100, annot=True, fmt=".2f", cmap="Blues", xticklabels=['Background', 'Bar'], yticklabels=['Background', 'Bar'])
...
if __name__ == "__main__":
    model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/bar/run_clamp_4/clamp_bar_SegNextV2_2024-12-27_accuracy_train_0.91559_loss_0.04419_epoch_12_lr_0.003_time_134.64M_alpha_0.7_beta_0.3_gamma_2_best.pth" 
    data_path = "/home/saqib/Projects/sebang_welding/Sebang_Latest_Dec24/clamp/circle_bar/new_dataset"
    save_dir = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/bar/run_clamp_4"  # Update with your desired folder to save outputs
    overlay_dir = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_3/clamp/bar/run_clamp_4/overlays_test"  # Directory to save overlays if enabled
