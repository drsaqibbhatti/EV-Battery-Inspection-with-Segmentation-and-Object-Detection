...
class PinholeDataset_filename(Dataset):
            ...
            if file.endswith(".json"):
                ...
                for label in jsonData['data']:
                    ...
                    for region in annotations:
                        # Handling both pinhole
                        if region['className'] in ['pinhole'] and region['type'] in ['PolyGon', 'PolyLine']:
    ...
    def __getitem__(self, index):
        ...
        original_filename = os.path.basename(imagePath)  # Extract the filename from the path
        ...
        cv_label_image_pinhole = np.zeros([original_height, original_width], dtype=np.uint8)
        ...
        for region in regions:
            ...
            # Fill based on class: Pinhole or Spatter
            if region['className'] == 'pinhole':
                if region['type'] == 'PolyGon':
                    cv2.fillPoly(cv_label_image_pinhole, [pts], (255))
                elif region['type'] == 'PolyLine':
                    cv2.polylines(cv_label_image_pinhole, [pts], isClosed=True, color=(255), thickness=stroke_width)

        if vFlip:
            cv_label_image_pinhole = cv2.flip(cv_label_image_pinhole, 1)
        if hFlip:
            cv_label_image_pinhole = cv2.flip(cv_label_image_pinhole, 0)
        ...
        # Resize masks
        cv_label_image_pinhole = cv2.resize(cv_label_image_pinhole, (resize_width, resize_height), interpolation=cv2.INTER_NEAREST)
        ...
        torch_label_image_pinhole = torch.tensor(cv_label_image_pinhole, dtype=torch.float32, device=device) / 255


        torch_label = torch.zeros([1, resize_height, resize_width], dtype=torch.float32, device=device).detach()
        torch_label[0] = torch_label_image_pinhole
        

        
        return torch_image, torch_label, original_filename
