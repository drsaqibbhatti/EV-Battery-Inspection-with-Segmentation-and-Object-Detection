...
class CircleDataset(Dataset):
            ...
            if file.endswith(".json"):
                ...
                for label in jsonData['data']:
                    ...
                    for region in annotations:
                        ...
                        if region['className'] in ['circle'] and region['type'] in ['PolyGon', 'PolyLine']:
    ...
    def __getitem__(self, index):
        ...
        cv_label_image_circle = np.zeros([original_height, original_width], dtype=np.uint8)
        ...
        for region in regions:
            ...
            if region['className'] == 'circle':
                if region['type'] == 'PolyGon':
                    cv2.fillPoly(cv_label_image_circle, [pts], (255))
                elif region['type'] == 'PolyLine':
                    cv2.polylines(cv_label_image_circle, [pts], isClosed=True, color=(255), thickness=stroke_width)

        if vFlip:
            cv_label_image_circle = cv2.flip(cv_label_image_circle, 0)
        if hFlip:
            cv_label_image_circle = cv2.flip(cv_label_image_circle, 1)
        ...
        # Resize masks
        cv_label_image_circle = cv2.resize(cv_label_image_circle, (resize_width, resize_height), interpolation=cv2.INTER_NEAREST)
        


        # Convert the image to tensor
        torch_image = self.transform(image)
        # print("Shape of torch_image RGB:", torch_image.shape)
        ################ BGR #########################
        torch_image = torch_image[[2, 1, 0], :, :]
        ##############################################
        # print("Shape of torch_image BGR:", torch_image.shape)
        torch_label_image_circle = torch.tensor(cv_label_image_circle, dtype=torch.float32, device=device) / 255


        torch_label = torch.zeros([1, resize_height, resize_width], dtype=torch.float32, device=device).detach()
        torch_label[0] = torch_label_image_circle
