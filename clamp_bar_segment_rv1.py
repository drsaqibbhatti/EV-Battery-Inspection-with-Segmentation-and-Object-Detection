...
def train(load_pre_trained_model=True):
    ...
    trainDataset = BarDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/clamp/bar",
    ...
    validDataset = BarDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/clamp/bar",
    ...
    dataset_name = "clamp_bar"
    build_date = str(date.today())
    model_name = f"{dataset_name}_{model_arch}_{build_date}"
    date_dir = os.path.join(Base_dir, 'trainedModel/Phase_2025/clamp/bar')
    ...
    run_dir = os.path.join(date_dir, f"run_clamp_{run_number}_H{imageHeight}_W{imageWidth}")
    os.makedirs(run_dir)
    ...
    if load_pre_trained_model:
        ...
        pretrained_model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_2025/clamp/bar/run_clamp_1_H256_W1024/clamp_bar_SegNextV2_2025-03-24_Acc_0.90751_loss_0.04862_E83_time_1188.77M_H256_W1024_best.pth"
    ...
    for epoch in range(epochs):
        ...
        if avg_acc > best_train_acc:
            ...
            best_model_path = os.path.join(run_dir,f"{model_name}_Acc_{best_train_acc:.5f}_loss_{avg_cost:.5f}_E{epoch+1}_time_{Total_training_time / 60:.2f}M_H{imageHeight}_W{imageWidth}_best.pth")
    ...
    last_model_path = os.path.join(run_dir,f"{model_name}_Acc_{avg_acc:.5f}_loss_{avg_cost:.5f}_E{epoch+1}_time_{Total_training_time / 60:.2f}M_H{imageHeight}_W{imageWidth}_last.pth")
