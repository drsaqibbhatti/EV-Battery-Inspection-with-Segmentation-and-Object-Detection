...
def train(load_pre_trained_model=True):
    ...
    trainDataset = WeldingWidthDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Cu/weld",
    ...
    validDataset = WeldingWidthDataset(path="/home/saqib/Projects/sebang_welding/Sebang_Latest_Mar25/SV_Busbar/Busbar_Cu/weld",
    ...
    pretrained_model_path = "/home/saqib/Projects/sebang_welding/trainedModel/Phase_2025/busbar_cu/width/run_2_H320_W640/busbar_cu_width_SegNext_2025-03-24_accuracy_Acc_0.976760919_loss_0.011780242_E_28_time_210.22M_H320_W640_best.pth"
    ###################################################################################################################################
    ...
    model_arch = "SegNext"
    dataset_name = "busbar_cu_width"
    build_date = str(date.today())
    model_name = f"{dataset_name}_{model_arch}_{build_date}"
    date_dir = os.path.join(Base_dir, 'trainedModel/Phase_2025/busbar_cu/width')
    ...
    run_dir = os.path.join(date_dir, f"run_{run_number}_H{imageHeight}_W{imageWidth}")
    os.makedirs(run_dir)
    ...
    for epoch in range(epochs):
        ...
        if avg_acc > best_train_acc:
            ...
            best_model_path = os.path.join(run_dir,f"{model_name}_Acc_{best_train_acc:.9f}_loss_{avg_cost:.9f}_E_{epoch+1}_time_{Total_training_time / 60:.2f}M_H{imageHeight}_W{imageWidth}_best.pth")
    ...
    last_model_path = os.path.join(run_dir,f"{model_name}_Acc_{avg_acc:.9f}_loss_{avg_cost:.9f}_E_{epoch+1}_time_{Total_training_time / 60:.2f}M_H{imageHeight}_W{imageWidth}_last.pth")
